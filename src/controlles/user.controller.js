...
const registerUser = asyncHandler( async (req, res) => {
    ...
    if (!createdUser){
        throw new ApiError(500, "Somethis went wrong while reqisterning user")
    }
...
const logoutUser = asyncHandler(async(req, res) => {
    ...
    return res.status(200).clearCookie("accessToken").clearCookie("refreshToken").json(new ApiResponse(
        200, {}, "User logged Out"
    ))
})
...
const getCurrentUser = asyncHandler(async(req, res) => {
    return res.status(200).json(new ApiResponse(200, req.user, "Current user feched successfully"))
})
...
const updateAccountDetails = asyncHandler(async(req, res) => {
    ...
    if (!fullname && !email){
     ...
     const user = await User.findByIdAndUpdate(req.user?._id,{
        $set:{
            fullname,   // or fullname: fullname. if both are same then we can write only one
            email: email //only 'email' is also fine
        }
